...
velcaminicio = 3; --A que velocidad de camara inicias la cancion, el predeterminado es 1
...
mediocamara = false; -- ACTIVA O DESACTIVA - true = camara empieza desde el medio, luego tienes que cambiarlo con el evento "p" para que sea normal
...
function onEvent(n,v1,v2)
    ...
    if n == "roval" then
        ...
        if v1 == 'hurra' then
            xbief = getProperty('boyfriend.x')
            xdada = getProperty('dad.x')
            ybief = getProperty('boyfriend.y')
            ydada = getProperty('dad.y')
            scalexbief = getProperty('boyfriend.scale.x')
            scalexdada = getProperty('dad.scale.x')
            scaleybief = getProperty('boyfriend.scale.y')
            scaleydada = getProperty('dad.scale.y')
        end
        ...
        if usarextra == true then
            ...
            if v1 == 'angulocam' then
                ...
                doTweenAngle('camarangul'..gradoc, 'camera', gradoc, velc, 'linear')
            end
...
function onUpdate()
    if boyfriendName == 'bf_aquino' then
