...
int = 15; -- INTENSIDAD DE MOVIMIENTO DE CAMARA, mas = intenso
opacidadhud = 0; -- La opacidad del hud que tiene AL INICIO DE LA CANCION, luego lo cambias con el evento "hudz" o "hud"
zoomextra = -0.05; --Coloca con cuanto zoom extra quieres que inicie la cancion, el predeterminado de extra es 0 (solo el que tiene el stage)
velcaminicio = 0.7; --A que velocidad de camara inicias la cancion, el predeterminado es 1
...
opacidadfondo = 0.9 -- si activas ATRASNEGRO 

dadx1 = -320; --Eje X de camara de dad (Menos = Izquierda  Mas = Derecha)
dady1 = -10; --Eje Y de camara de dad (Menos = Arriba  Mas = Abajo)
bfx1 = -450; --Eje X de camara de bf (Menos = Izquierda  Mas = Derecha)
bfy1 = -85; --Eje Y de camara de bf (Menos = Arriba  Mas = Abajo)
...
usarcolor = true --Funciones de cambio de color
...
function onCreatePost()
    ...
    if usarletras == true then
        ...
        setTextFont('texto', 'duxo.ttf') --LA FUENTE QUE SE USA
        ...
        makeLuaText('textob', 'aaaaaaaaaaaa', 1560, -140, -10)

        setTextFont('textob', 'duxo.ttf') --LA FUENTE QUE SE USA
        setScrollFactor('textob', 0.99, 0.99)
        setTextColor('textob', 'ffffff') --AJUSTAS EL COLOR DEL TEXTO EN MEDIO
        setProperty('textob.colorTransform.greenOffset', -217)
        setProperty('textob.colorTransform.redOffset', -50)
        setProperty('textob.colorTransform.blueOffset', -43)
        setTextSize('textob', 500); -- EL TAMANO DE LA LETRA
        setProperty('textob.alpha', 0)
        setObjectCamera("textob", 'game');
        setTextAlignment('textob', 'center')
        setObjectOrder('textob', getObjectOrder('negro') + 1)
        
        addLuaText('textob')

        makeLuaText('textoc', '', 1000, 460, 500)
        setTextFont('textoc', 'duxo.ttf') --LA FUENTE QUE SE USA
    ...
    if n == "roval" then
        ...
        if usarletras == true then
            ...
            if v1 == 'cantab' then --Activar para cambiar el texto de karaoke - Segundo texto
                ...
                doTweenY('textitob', 'textob', 0, 0.001)
...
function onTimerCompleted(tag, loops, loopsLeft)
    ...
    if usarextra == true then
        ...
        if tag == 'fintextob' then
            doTweenAlpha('textob', 'textob', 0, 0.5)
        end

        if tag == 'muevetextob' then
            doTweenY('textitoab', 'textob', -50, 0.7, 'quadOut')
        end
    end
end
