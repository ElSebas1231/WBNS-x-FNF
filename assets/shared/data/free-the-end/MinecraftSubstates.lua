...
function onGameOver()
    if not amIdead then
        if boyfriendName == 'Locochon-speedrun-nether' then
            amIdead = true
            triggerEvent('Play Animation', 'muerte', 'bf')
            setProperty('boyfriend.color', 16777215)
            setProperty('camHUD.visible', false)

            setProperty('Escenario.x', -300)
            setProperty('Escenario2.x', -300)
            setProperty('bloque3.x', -383)
            setProperty('bloque2.x', -383)
            setProperty('bloque1.x', -383)
