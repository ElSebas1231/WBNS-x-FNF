...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var options:Array<Dynamic> = [
		[true, 'NOTAS'],
		[true, 'Izq.', 'note_left', 'Nota Izq.'],
		[true, 'Abajo', 'note_down', 'Nota Abajo'],
		[true, 'Arriba', 'note_up', 'Nota Arriba'],
		[true, 'Dere.', 'note_right', 'Nota Dere.'],
		[true],
		[true, 'UI'],
		[true, 'Izq.', 'ui_left', 'UI Izq.'],
		[true, 'Abajo', 'ui_down', 'UI Abajo'],
		[true, 'Arriba', 'ui_up', 'UI Arriba'],
		[true, 'Dere.', 'ui_right', 'UI Dere.'],
		[true],
		[true, 'Reset', 'reset', 'Reset'],
		[true, 'Aceptar', 'accept', 'Aceptar'],
		[true, 'Volver', 'back', 'Volver'],
		[true, 'Pausar', 'pause', 'Pausar'],
		[false],
		[false, 'VOLUME'],
		[false, 'Ensord.', 'volume_mute', 'Ensord. Volumen'],
		[false, 'Subir', 'volume_up', 'Subir Volumen'],
		[false, 'Baja', 'volume_down', 'Bajar Volumen'],
		[false],
		[false, 'DEBUG'],
		[false, 'Tecla 1', 'debug_1', 'Tecla Debug #1'],
		[false, 'Tecla 2', 'debug_2', 'Tecla Debug #2']
	];
	...
	override function update(elapsed:Float)
	{
		...
		if(!binding)
		{
			...
			if(FlxG.keys.justPressed.UP || FlxG.gamepads.anyJustPressed(DPAD_UP) || FlxG.gamepads.anyJustPressed(LEFT_STICK_DIGITAL_UP)) {
				updateText(-shiftMult);
				holdTime = 0;
			}

			if(FlxG.keys.justPressed.DOWN || FlxG.gamepads.anyJustPressed(DPAD_DOWN) || FlxG.gamepads.anyJustPressed(LEFT_STICK_DIGITAL_DOWN)) {
				updateText(shiftMult);
				holdTime = 0;
			}
			...
			if(controls.UI_DOWN || controls.UI_UP)
			{
				...
				if(holdTime > 0.5 && checkNewHold - checkLastHold > 0)
				{
					...
					updateText((checkNewHold - checkLastHold) * (controls.UI_UP ? -shiftMult : shiftMult));
				}
			}
			...
			{
				FlxG.sound.play(Paths.sound('scrollMenu'), 0.5);
				updateText(-shiftMult * FlxG.mouse.wheel);
			}
			...
			if(FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A))
			{
				if(options[curOptions[curSelected]][1] != defaultKey)
				{
					...
					bindingText = new Alphabet(FlxG.width / 2, 160, "Cambiando:  " + options[curOptions[curSelected]][3], false);
					bindingText.alignment = CENTERED;
					add(bindingText);
					
					bindingText2 = new Alphabet(FlxG.width / 2, 340, "Mantén ESC para cancelar\nMantén BckSpc para borrar", true);
