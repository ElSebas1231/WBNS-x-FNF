...
class ModSettingsSubState extends BaseOptionsMenu
{
	...
	public function new(options:Array<Dynamic>, folder:String, name:String)
	{
		...
		title = '';
		//title = name;
		rpcTitle = 'Ajustes de Mods ($name)'; //for Discord Rich Presence
		...
		try
		{
			for (option in options)
			{
				var newOption = new Option(
					...
					option.description != null ? option.description : 'Sin descripción.',
					option.save,
					option.type,
					option.options
				);
		...
		{
			var errorTitle = 'Nombre del mod: ' + folder;
			var errorMsg = 'Ocurrió un erro: $e';
			#if windows
