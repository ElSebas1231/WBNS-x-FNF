...
class MusicPlayer extends FlxGroup 
{
	...
	var songImage:String = 'wbnsxfnf'; // Initial anim
	var songPortrait:FlxSprite;

	var portraitSongs:Array<String> = [
		'animal conflict', 'ay wtf', 'baneado', 'beat the gf', 'boneless wings', 'code c3jo',
		'creepers', 'dad', 'dinamica', 'disco fish', "don't look up", 'double a', 'era bait',
		'free the end', 'goty', 'how bad can i be', 'identical', 'late night', 'laughs', 'say it',
		"let's go compota", 'loco classic encore', 'lore (soarinng mix)', 'made in 2002',
	...
	public function new(instance:FreeplayState)
	{
		...
		songPortrait = new FlxSprite(680, -20);
		songPortrait.frames = Paths.getSparrowAtlas('soundtrack');
		songPortrait.scale.set(0.5, 0.5);
		songPortrait.antialiasing = ClientPrefs.data.antialiasing;
		for (i in 0...portraitSongs.length){
			songPortrait.animation.addByPrefix(portraitSongs[i], portraitSongs[i]);
		}
		songPortrait.animation.play(songImage);
		add(songPortrait);
	...
	override function update(elapsed:Float)
	{
		...
		if (paused && !wasPlaying)
			songTxt.text = 'Sonando: ' + instance.songs[FreeplayState.curSelected].songName + ' (Pausado)';
		else
			songTxt.text = 'Sonando: ' + instance.songs[FreeplayState.curSelected].songName;
	...
	{
		songImage = 'wbnsxfnf';
		freeplaySong = instance.songs[FreeplayState.curSelected].songName.toLowerCase();
		var freeplaySongLenght:Int = instance.songs[FreeplayState.curSelected].songName.length;

		FlxG.autoPause = (!playingMusic && ClientPrefs.data.autoPause);
		active = visible = playingMusic;

		instance.scoreBG.visible = instance.diffText.visible = instance.scoreText.visible = instance.completitionText.visible = !playingMusic; //Hide Freeplay texts and boxes if playingMusic is true
		...
		if (playingMusic)
		{
			instance.bottomText.text = "Presiona SPACE para pausar / Presiona ESCAPE para salir del reproductor / Presiona R para reiniciar la canción";
			positionSong();
			...
			for (i in 0...portraitSongs.length){
				if (freeplaySong == portraitSongs[i]) {
					songImage = portraitSongs[i];
				}
			}
			
			if (freeplaySong.contains('lore (soarinng mix)')) {
				songImage = 'lore (soarinng mix)';
			}
			songPortrait.animation.play(songImage);
			
			updateTimeTxt();

			if (freeplaySongLenght >= 14 && freeplaySongLenght <= 16) {
				songPortrait.x = 720;
				songPortrait.y = -40;
	...
	function updatePlaybackTxt()
	{
		...
		playbackTxt.text = 'Velocidad: ' + text + 'x';
	}
